    ...
    try {
        ...
        if (decodeToken && typeof decodeToken !== 'string' && 'exp' in decodeToken) {
            ...
            if (Date.now() > exp! * 1000) {
                try {
                    const verifyRefreshToken = jwt.verify(refreshToken, process.env.JWT_REFRESH_TOKEN_SECRET!) as jwt.JwtPayload
                    const newAccessToken = jwt.sign({ id: verifyRefreshToken.id, role: verifyRefreshToken.role }, process.env.JWT_ACCESS_TOKEN_SECRET!, { expiresIn: "30m" })
                ...
                try {
                    const verifyAccessToken = jwt.verify(accessToken, process.env.JWT_ACCESS_TOKEN_SECRET!) as jwt.JwtPayload
                    return next();
                } catch (error) {
