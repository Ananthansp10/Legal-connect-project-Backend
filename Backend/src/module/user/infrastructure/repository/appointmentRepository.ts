...
export class AppointmentRepository implements IAppointmentRepository {
  async findAppointmentByUserId(
    ...
    if (appointmentStatus == AppointmentStatus.PENDING) {
      result = await appointmentModel
        .find({ userId: userId, appointmentStatus: AppointmentStatus.PENDING })
        .skip(startIndex)
        .limit(limit);
    } else if (appointmentStatus == AppointmentStatus.ACCEPTED) {
      result = await appointmentModel
        .find({ userId: userId, appointmentStatus: AppointmentStatus.ACCEPTED })
        .skip(startIndex)
        .limit(limit);
      ...
      result = await appointmentModel
        .find({ userId: userId, appointmentStatus: AppointmentStatus.REJECTED })
        .skip(startIndex)
        .limit(limit);
    } else if (appointmentStatus == AppointmentStatus.UPCOMING) {
      result = await appointmentModel
        .find({ userId: userId, appointmentStatus: AppointmentStatus.BOOKED })
        .skip(startIndex)
        .limit(limit);
    }
  ...
  async cancelAppointment(appointmentId: Types.ObjectId): Promise<void> {
    await appointmentModel.findByIdAndUpdate(appointmentId, {
      $set: { appointmentStatus: AppointmentStatus.CANCELLED },
    });
  }
