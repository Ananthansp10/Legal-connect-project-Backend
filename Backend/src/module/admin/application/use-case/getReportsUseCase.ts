...
export class GetReportsUseCase implements IGetReportsUseCase {
  ...
  async execute(
    ...
    let totalSubscribedLawyers =
      (await this._reportsRepo.getTotalSubscribedLawyers()) || 0;
    let subscriptionPlanReport =
      (await this._reportsRepo.getSubscriptionPlanReport()) || [];
    ...
    let revenueDateChart =
      (await this._reportsRepo.getRevenueDateChart(revenueDateRange)) || [];
    let specializationChart =
      (await this._reportsRepo.getSpecializationChart(specializationType)) ||
      [];
