...
export class AdminLawyerManagementController {
    ...
    async getUnverifiedLawyers(req: Request, res: Response) {
        try {
            const result: ILawyerResponse[] | null = await this._getUnverifiedLawyerApplication.execute()
    ...
    async verifyLawyer(req: Request, res: Response) {
        try {
            const result: boolean = await this._verifyLawyerApplication.execute(req.params.lawyerId, req.params.status, req.params.reason)
            if (result) {
    ...
    async getLawyers(req: Request, res: Response) {
        try {
            const result: { data: ILawyerResponse[], totalData: number } | null = await this._getLawyersApplication.execute(parseInt(req.query.page as string), parseInt(req.query.limit as string))
    ...
    async verifyLawyerStatus(req: Request, res: Response) {
        try {
            const result: boolean = await this._verifyLawyerStatusApplication.execute(req.params.lawyerId, req.params.status)
            if (result) {
    ...
    async getLawyerProfile(req: Request, res: Response) {
        try {
            const result = await this._getLawyerProfileDataUseCase.execute(new mongoose.Types.ObjectId(req.params.lawyerId))
