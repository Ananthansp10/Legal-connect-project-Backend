...
export class AdminUserManagementController {
    ...
    async getUsers(req: Request, res: Response) {
        try {
            const result: { data: IUserResponse[], totalData: number } | null = await this._getUserApplication.execute(parseInt(req.params.startIndex), parseInt(req.params.limit))
    ...
    async verifyUserStatus(req: Request, res: Response) {
        try {
            const result: boolean = await this._verifyUserStatusApplication.execute(req.params.userId, req.params.status)
            if (result) {
    ...
    async getUserProfile(req: Request, res: Response) {
        try {
            const result = await this._getUserProfileDataUseCase.execute(new mongoose.Types.ObjectId(req.params.userId))
