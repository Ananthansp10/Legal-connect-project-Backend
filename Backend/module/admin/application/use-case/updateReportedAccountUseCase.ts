...
export class UpdateReportedAccountUseCase
  implements IUpdateReportedAccountStatusUseCase
{
  constructor(private _reportedAccountRepo: IReportedAccountsRepository) {}

  async execute(reportedAccountId: Types.ObjectId): Promise<void> {
    await this._reportedAccountRepo.updateReportAccountStatus(
      reportedAccountId,
    );
  }
}
