...
export class GetAppointmentsUseCase implements IGetAppointmentsUseCase {
    ...
    async execute(appointmentStatus: string, startIndex: number, limit: number): Promise<{ appointments: AppointmentsData[] | [], totalAppointments: number } | []> {

        const appointments = await this._appointmentRepo.findAppointments(appointmentStatus, startIndex, limit)
        ...
        return { appointments: appointmentsDetails || [], totalAppointments: appointments.totalAppointments }
    }
}