...
export class GetSpecializationRepository
  implements IGetSpecializationRepository
{
  async getSpecialization(
    ...
    let specializations = await specializationModel
      .find({ isDeleted: false })
      .skip(startIndex)
      .limit(limit);
    let totalSpecializations = await specializationModel.countDocuments({
      isDeleted: false,
    });
