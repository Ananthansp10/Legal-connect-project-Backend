...
export class PaymentController {
    ...
    async createRazorpayOrder(req: Request, res: Response) {
        try {
            const order = await this._createRazorpayOrderUseCase.execute(req.body.planId, req.body.price)
            res.status(AppStatusCode.SUCCESS_CODE).json({ success: true, message: "Razorpay order created successfully", data: { ...order, planId: req.body.planId } })
        } catch (error) {
