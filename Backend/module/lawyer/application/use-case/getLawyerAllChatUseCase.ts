...
export class GetLawyerAllChatUseCase implements IGetLawyerAllChatsUseCase{
    ...
    async execute(lawyerId: Types.ObjectId): Promise<ChatsDto[] | null> {
        let chats=await this._chatRepo.findAllChats(lawyerId)
        if(!chats || chats.length==0){
            return null
        }
        let chatDetails=await Promise.all(
            chats.map(async(chat)=>{
                let userDetails=await this._chatRepo.findUserDetails(chat.participants[0])
                return{
                    ...
                    unreadCount:chat.messages.filter((msg)=>msg.receiverId.toString()==lawyerId.toString() && !msg.isRead).length
                }
            })
        )
