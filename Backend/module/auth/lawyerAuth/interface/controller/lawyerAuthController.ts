...
export class LawyerAuthController {
    ...
    async registerLawyer(req: MulterRequest, res: Response): Promise<void> {
        try {
            ...
            const result = await this._lawyerSignupApplication.registerLawyer(requestObj)
    ...
    async siginLawyer(req: Request, res: Response, cookieTokenService: ICookieTokenService) {
        try {
            const { lawyerDetails, accessToken, refreshToken } = await this._lawyerSigninApplication.execute(req.body.email, req.body.password)
