...
export class AdminSigninUseCase implements IAdminSigninUseCase {
    ...
    async execute(data: adminSignInDto): Promise<{ accessToken: string, refreshToken: string }> {
        try {
            ...
            const accessToken = this._tokenGenerateService.generateAccessToken({ id: 'admin', role: 'admin' })
            const refreshToken = this._tokenGenerateService.generateRefreshToken({ id: 'admin', role: 'admin' })
