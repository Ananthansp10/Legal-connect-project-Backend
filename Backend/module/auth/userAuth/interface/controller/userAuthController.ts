...
export class UserAuthController {
    ...
    async registerUser(req: Request, res: Response): Promise<void> {
        try {
            const result: Omit<IUserSignup, 'isBlock' | 'isActive' | 'password'> | null = await this._userSignupApplication.registerUser(req.body)
    ...
    async forgotPassword(req: Request, res: Response): Promise<void> {
        try {
            const result = await this._forgotPasswordApplication.execute(req.body.email)
    ...
    async signin(req: Request, res: Response, cookieTokenService: ICookieTokenService): Promise<void> {
        try {
            const { userData, accessToken, refreshToken }: UserSigninDto = await this._userSigninAplication.execute(req.body.email, req.body.password)
    ...
    async googleAuthentication(req: Request, res: Response, cookieTokenService: ICookieTokenService): Promise<void> {
        try {
            const { userData, accessToken, refreshToken }: UserSigninDto = await this._googleAuthApplication.execute(req.user as GoogleAuthEntity)
            cookieTokenService.setAuthCookie(res, accessToken, refreshToken)
