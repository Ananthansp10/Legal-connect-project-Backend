...
import { IGetTodaysAppointmentsUseCase, TodaysAppointmentResponseDataDto } from "../use-case-interface/IGetTodaysAppointmentUseCase";
...
export class GetTodaysAppointmentsUseCase implements IGetTodaysAppointmentsUseCase {
    ...
    async execute(userId: Types.ObjectId): Promise<TodaysAppointmentResponseDataDto[] | []> {
        const appointments = await this._appointmentRepo.getTodaysAppointment(userId, new Date().toISOString().split('T')[0])
