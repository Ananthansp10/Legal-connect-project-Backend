...
export class ChatRepository implements IChatRepository{
    ...
    async findChatConnection(userId: Types.ObjectId, lawyerId: Types.ObjectId): Promise<IChatEntity | null> {
        return await chatModel.findOne({participants:{$all:[userId,lawyerId]}})
    }

    async addMessage(userId: Types.ObjectId, lawyerId: Types.ObjectId, message: Messages): Promise<void> {
        await chatModel.updateOne({participants:{$all:[userId,lawyerId]}},{$push:{messages:message}})
    }
