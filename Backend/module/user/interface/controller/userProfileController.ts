...
export class UserProfileController {
    ...
    async getUserProfile(req: Request, res: Response) {
        try {
            const result: UserProfileMapper | null = await this._userGetProfile.execute(req.params.userId)
    ...
    async editUserProfile(req: Request, res: Response) {
        try {
            const imageUrl = req?.file?.path
            const result: UserProfileMapper = await this._editUserProfile.execute(req.body.userId, req.body, imageUrl!)
